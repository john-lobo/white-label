...
import br.com.douglasmotta.whitelabeltutorial.domain.usercase.CreateProductUseCase
...
class AddProductViewModel @Inject constructor(
    ...
    fun createProduct(description :String, prince:String, imageUri: Uri?) = viewModelScope.launch {
         isFormValid = true
        _imageUriErrorResId.value = getDrawableResIdIfNull(imageUri)
        _descriptionFieldErrorResId.value = getErrorStringIdIfEmpty(description)
        _priceFieldErrorResId.value = getErrorStringIdIfEmpty(prince)

        if(isFormValid){
            try {
                val product = createproductUseCase(description,prince.fromCurrency(),imageUri!!)
    ...
    private fun getErrorStringIdIfEmpty(value:String):Int?{
